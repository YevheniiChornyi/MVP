...
public class Basketball implements Sport{
    ...
    public void defineWinnerTeam() {
        ...
        String team1 = basketballPlayers.get(0).getTeamName(); String team2 = null;
        int team1score = 0; int team2score  = 0;

        for (BasketballPlayer player:
                basketballPlayers) {
            if (!player.getTeamName().equals(team1)) {
                team2 = player.getTeamName(); break;
            }
        }
        for (BasketballPlayer player:
                basketballPlayers) {
        ...
        if (team1score > team2score) {winner = team1; return;}
        winner = team2;
    }
    ...
    public void parseLine(List<String> allLines) throws WrongFileFormatException {
        for (String string:
            ...
            regex(split[0]); regex(split[1]); regex(split[3]);
            String name = split[0];
